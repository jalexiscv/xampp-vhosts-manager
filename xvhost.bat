...
rem ---------------------------------------------
rem Try PHP from the same XAMPP installation first
if exist "%~dp0..\php\php.exe" (
    set "phpBin=%~dp0..\php\php.exe"
) else (
    for /F "tokens=* USEBACKQ" %%v in (`where php 2>nul`) do (
        if exist "%%v" set "phpBin=%%v" & goto phpFound
    )
    goto phpBinNotFound
)
:phpFound
...
echo Make sure you have XAMPP installed alongside this tool, or add the path
echo to your PHP directory into Windows Path Environment Variable.
echo Expected: %~dp0..\php\php.exe
call :clearEnvVars
exit /B 1
...
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "install"
...
rem ---------------------------------------------
:newHost
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "newHost" "%~2"
call :clearEnvVars
exit /B %errorLevel%

rem ---------------------------------------------
:showHostInfo
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "showHostInfo" "%~2"
call :clearEnvVars
exit /B %errorLevel%

rem ---------------------------------------------
:listHosts
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "listHosts"
call :clearEnvVars
exit /B %errorLevel%

rem ---------------------------------------------
:removeHost
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "removeHost" "%~2"
call :clearEnvVars
exit /B %errorLevel%

rem ---------------------------------------------
:addSSL
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "addSSL" "%~2"
call :clearEnvVars
exit /B %errorLevel%

rem ---------------------------------------------
:removeSSL
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "removeSSL" "%~2"
call :clearEnvVars
exit /B %errorLevel%

rem ---------------------------------------------
:changeDocRoot
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "changeDocRoot" "%~2"
call :clearEnvVars
exit /B %errorLevel%

rem ---------------------------------------------
:registerPath
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "registerPath"
call :clearEnvVars
exit /B %errorLevel%

rem ---------------------------------------------
:grantPermsWinHosts
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "grantPermsWinHosts"
call :clearEnvVars
exit /B %errorLevel%

rem ---------------------------------------------
:stopApache
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "stopApache"
call :clearEnvVars
exit /B %errorLevel%

rem ---------------------------------------------
:startApache
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "startApache"
call :clearEnvVars
exit /B %errorLevel%

rem ---------------------------------------------
:restartApache
"%phpBin%" -n -d output_buffering=0 "%XVHM_PHP_CONTROLLER%" "restartApache"
call :clearEnvVars
exit /B %errorLevel%

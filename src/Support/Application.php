...
class Application
{
    ...
    final protected function addedCaCertToStore()
    {
        ...
        // Get Cert serial number (works with any Windows language)
        exec('CertUtil -enterprise -silent -verify "'. $this->paths['caCertDir'] .'\cacert.crt"', $arrOutput, $exitCode);

        foreach ($arrOutput as $line) {
            if (preg_match('/serial\s*(?:number|#|n[uo]m[ée]ro)?[:\s]+([a-f0-9]+)/i', $line, $matches)) {
        ...
        if ($certSerial) {
            // Try enterprise store first (domain-joined machines), then local machine store
            foreach (['-enterprise -verifystore', '-verifystore'] as $storeFlag) {
                $arrOutput = [];
                exec('CertUtil -silent ' . $storeFlag . ' Root ' . $certSerial, $arrOutput, $exitCode);

                foreach ($arrOutput as $line) {
                    if (strpos($line, $certSerial) !== false) {
                        return true;
                    }
                }
            }
    ...
    private function defineAppPaths()
    {
        ...
        } elseif (array_key_exists('SYSTEMROOT', $_SERVER)) {
            $this->paths['winHostsFile'] = realpath($_SERVER['SYSTEMROOT'] . '\\System32\\drivers\\etc\\hosts');
        } else {
            $this->paths['winHostsFile'] = realpath('C:\\Windows\\System32\\drivers\\etc\\hosts');
        }
